...
async function getCityCoordinates(city, retryCount = 0) {
  ...
  try {
    ...
    const response = await fetch(
      `https://restapi.amap.com/v3/geocode/geo?key=b86966801b1b0eeee797f1bd52a17575&address=${encodedCity}&output=JSON`
    );
...
async function getTransitRoute(origin, destination, city1, city2, retryCount = 0) {
  ...
  try {
    const response = await fetch(
      `https://restapi.amap.com/v3/direction/transit/integrated?key=b86966801b1b0eeee797f1bd52a17575&origin=${origin}&destination=${destination}&city=${encodeURIComponent(city1)}&cityd=${encodeURIComponent(city2)}&strategy=3&extensions=all`
    );
...
// API端点
app.post('/api/calculate-optimal-cities', async (req, res) => {
  try {
    const { cityA, cityB, maxResults = 5 } = req.body;

    console.log(`接收到请求: cityA=${cityA}, cityB=${cityB}, maxResults=${maxResults}`);

    if (!cityA || !cityB) {
      return res.status(400).json({ error: '缺少必要参数：cityA 和 cityB 为必填项' });
    }

    if (cityA === cityB) {
      return res.status(400).json({ error: '两个城市不能相同，请提供不同的出发地和目的地' });
    }
    ...
    try {
      ...
      if (!nearbyCities || nearbyCities.status !== '1' || !nearbyCities.pois || nearbyCities.pois.length === 0) {
        return res.json({ recommendations: [], message: '未找到候选城市' });
      }
      ...
      if (recommendations.length === 0) {
        return res.json({ recommendations: [], message: '未找到合适的约会地点' });
      }
      ...
      return res.status(500).json({ error: '计算失败，请检查城市名称是否正确' });
    }
  } catch (error) {
